...
class TextParser(NodeVisitor):
    ...
    def visit_Reference(self, n, (Identifier, _1, Subs, _2)):
        ...
        if Subs:
            ...
            if len(InterSub)==1:
                if Subs.count('!')==0:
                    return "functions.shorthander(%s,%s.dimension_dir,loc_dimension_dir,_subscript_dict)"%(getelemstr,Identifier)
                else:
                    return getelemstr
            else:
                if re.search("!",Subs) and Subs.count("!")==1:
                    ...
                    getelemstr = "functions.shorthander(%s,%s.dimension_dir,loc_dimension_dir,_subscript_dict)"%(getelemstr,Identifier)
                return getelemstr
        else:
            return Identifier+'()'
