...
def read_vensim(mdl_file,params={}):
    ...
    if params:
        ...
        for key, value in params.iteritems():
            functiontochange=re.findall(r'(def %s\(\)\:(.|\n)*?return output\s+%s.dimension_dir[^\n]*)'%(key,key),pythonfile)[0][0]
            if isinstance(value,np.ndarray):
                value=str(value)
                value=re.sub(r'(((?<=[0-9])\s+(?=[0-9]))|((?<=\])\s+(?=\[)))',',',value)
            value=str(value)
            functiontochange=re.sub(r'output = (.|\n)*return output\s*', 'output = np.array(%s) \n     return output\n'%value,functiontochange)
...
def read_jitia(mdl_file,params={}):
    from translators import translate_jitia
    """ Construct a model from Jitia `.dyn` file. """
    py_model_file = translate_jitia(mdl_file)
    if params:
        pythonfile=open(py_model_file, 'r')
        pythonfile=' '.join(pythonfile)
        for key, value in params.iteritems():
...
def load(py_model_file):
    ...
    nocache = (['_t', 'time_step', 'time', 'initial_time', 'final_time', 'division', 'functions', 'namedarray', 'np', 'saveper',
    ...
    cache_list = filter(lambda x: not x.startswith('__') and x not in nocache and len(inspect.getargspec(getattr(components,x))[0])==0, dir(components))
...
class PySD(object):
    ...
    def run(self, params={}, return_columns=[], return_timestamps=[],
            initial_condition='original', collect=False, flatten_subscripts=True):
    ...
    def reset_state(self,retry_flag=False):
    ...
    def _flatten_dataframe(self, dataframe):
        ...
        def dataframeexpand(pddf):
            ...
            for pos,name in enumerate(pddf.columns):
                ...
                if isinstance(pddf[name].loc[pddf.index[0]], np.ndarray):
