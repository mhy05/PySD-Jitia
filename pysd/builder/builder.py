...
class Builder(object):
    ...
    def add_stock(self, identifier, sub, expression, initial_condition, subi=['']):
        ...
        self.add_flaux('_' + identifier + '_init',subi,initial_condition)
        self.add_flaux('_d' + identifier + '_dt',sub,expression)
    ...
    def add_flaux(self, identifier, sub, expression, doc=''):
        ...
        if re.search('@',funcset):
            funcset=re.sub(r'@.*?(and|or).*?@',r' \1 ',funcset)    
        
        if identifier!='initial_time':
            funcstr = templates['flaux'].substitute(identifier=identifier,
                                                    expression=funcset,
                                                    docstring=docstring)
        else:
            funcstr = templates['initial'].substitute(identifier=identifier,
                                                      expression=funcset,
                                                      docstring=docstring)
                                                    
        if sub[0] != '':  # todo: make less brittle
            funcstr += '%s.dimension_dir = '%identifier+directory.__repr__()+'\n'  # todo: do we like 'dimension_dictionary' as a name?
        # funcstr = re.sub('\[\]','[:]',funcstr)
        self.body.append(funcstr)
    ...
    def add_n_delay(self, delay_input, delay_time, initial_value, order, sub):
        delay_input=re.sub(r'[\(\)\@]','',delay_input.replace('functions.shorthander(','').split(',')[0])
        delay_time=re.sub(r'[\(\)\@]','',delay_time.replace('functions.shorthander(','').split(',')[0])
        ...
        naked_input = re.sub(r'\[[^\[]*\]','',delay_input)
        naked_input = re.sub(r'[\[\]\:\(\)\*\/\+\-\.]','',naked_input)        
        if re.match(r'\d',naked_input):
            naked_input='number_'+naked_input        
        delay_input=re.sub(r'(\w*)(?!(\(\)|\w))',lambda s: s.group(1)+'()' if re.search(r'[a-z]',s.group(1)) else s.group(1),delay_input)
        naked_delay = re.sub(r'[\[\]\:\(\)\*\/\+\-\.]','',delay_time)
        delay_name = '%s_delay_%s'%(naked_input, naked_delay)
        delay_time=re.sub(r'(\w*)(?!(\(\)|\w))',lambda s: s.group(1)+'()' if re.search(r'[a-z]',s.group(1)) else s.group(1),delay_time)        
    ...
    def add_n_smooth(self, smooth_input, smooth_time, initial_value, order, sub):        
        smooth_input=re.sub(r'[\(\)\@]','',smooth_input)
        smooth_time=re.sub(r'[\(\)\@]','',smooth_time)
        """Constructs stock and flow chains that implement the calculation of
        ...
        naked_input = re.sub(r'\[[^\[]*\]','',smooth_input)
        naked_input = re.sub(r'[\[\]\:\(\)\*\/\+\-\.]','',naked_input)
        if re.match(r'\d',naked_input):
            naked_input='number_'+naked_input
        smooth_input=re.sub(r'(\w*)(?!(\(\)|\w))',lambda s: s.group(1)+'()' if re.search(r'[a-z]',s.group(1)) else s.group(1),smooth_input)
        naked_smooth = re.sub(r'[\[\]\:\(\)\*\/\+\-\.]','',smooth_time)
        smooth_name = '%s_smooth_%s'%(naked_input, naked_smooth)
        smooth_time=re.sub(r'(\w*)(?!(\(\)|\w))',lambda s: s.group(1)+'()' if re.search(r'[a-z]',s.group(1)) else s.group(1),smooth_time)
        ...
        for i in range(2, order+1):
            prev = '%s_stock_%i_of_%i()'%(smooth_name, i-1, order)
            current = '%s_stock_%i_of_%i()'%(smooth_name, i, order)
