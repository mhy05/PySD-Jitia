...
from pysd.namedarray import namedarray
...
def ${identifier}(x):
    ...
    if isinstance(x,int):
        fin_out = np.ndarray(np.shape(output))
        for pos,val in np.ndenumerate(output):
            fin_out[pos] = np.interp(x,val['x'],val['y'])
    else:
        if isinstance(x,namedarray):
            x=x['array']
        fin_out = np.ndarray(np.shape(x))
        for pos,val in np.ndenumerate(x):
            if pos:
                fin_out[pos] = np.interp(val,output[pos]['x'],output[pos]['y'])
            else:
                fin_out = np.interp(val,output['x'],output['y'])
    return fin_out
""")
